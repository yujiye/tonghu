...
CREATE TABLE `material` (
  `id` bigint(20) NOT NULL AUTO_INCREMENT COMMENT '记录id(自增主键)',
  `materail_name` varchar(50) COLLATE utf8_bin NOT NULL COMMENT '素材名称',
  `materail_type` tinyint(4) NOT NULL COMMENT '素材类型，目前只支持图片和视频两种素材 1 图片 2 视频',
  `attach_id` bigint(20) NOT NULL COMMENT '附件id，与附件表存在主外键关系',
  `materail_desc` varchar(200) COLLATE utf8_bin DEFAULT NULL COMMENT '素材简介',
  `create_userid` bigint(20) NOT NULL COMMENT '记录创建userid',
  `update_userid` bigint(20) DEFAULT NULL COMMENT '记录修改userid',
  `create_time` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '记录创建时间',
  `update_time` timestamp NOT NULL DEFAULT '0000-00-00 00:00:00' COMMENT '记录最后修改时间',
  PRIMARY KEY (`id`),
  UNIQUE KEY `IDX_MATERIAL_NAME` (`materail_name`),
  KEY `FK_REF_MATERIAL_ATTACHMENT` (`attach_id`),
  CONSTRAINT `FK_REF_MATERIAL_ATTACHMENT` FOREIGN KEY (`attach_id`) REFERENCES `attachment` (`id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COLLATE=utf8_bin COMMENT='素材数据表，存储图片和视频等素材';
...
insert  into `resources`(`id`,`resource_name`,`resource_url`,`module_id`,`sort_num`,`is_restricted`,`is_menu`,`create_time`,`create_userid`,`update_time`,`update_userid`,`note`) values (1,'首页','/warning.do',1,1,0,1,'2018-06-15 20:37:00',1,'0000-00-00 00:00:00',NULL,NULL),(2,'摄像头','/projectList.do',2,1,1,1,'2018-06-15 21:07:40',1,'0000-00-00 00:00:00',NULL,NULL),(3,'门禁','/projectCreate.do',2,2,1,1,'2018-06-15 21:10:06',1,'0000-00-00 00:00:00',NULL,NULL),(4,'无人机','/projectStatInfo.do',2,3,1,1,'2018-06-15 21:12:41',1,'0000-00-00 00:00:00',NULL,NULL),(5,'单兵系统','/projectShowPass.do',2,4,1,1,'2018-06-15 21:13:48',1,'0000-00-00 00:00:00',NULL,NULL),(6,'Mock数据','/mockDataManage.do',5,1,1,1,'2018-06-15 21:21:56',1,'0000-00-00 00:00:00',NULL,NULL),(7,'资源数据','/materialManage.do',5,2,1,1,'2018-06-15 21:21:56',1,'0000-00-00 00:00:00',NULL,NULL),(8,'用户管理','/userManage.do',6,1,1,1,'2018-06-15 21:21:56',1,'0000-00-00 00:00:00',NULL,NULL),(9,'查看当前用户信息','/currentUserInfo.do',6,2,0,0,'2018-06-15 21:27:25',1,'0000-00-00 00:00:00',NULL,NULL),(10,'设置当前用户信息','/setCurrentUserInfo.do ',6,3,0,0,'2018-06-15 21:28:25',1,'0000-00-00 00:00:00',NULL,NULL),(11,'修改当前用户密码','/setCurrentUserPassInfo.do',6,4,0,0,'2018-06-15 21:29:24',1,'0000-00-00 00:00:00',NULL,NULL),(12,'当前在线用户','/onlineUserList.do',6,5,1,0,'2018-06-15 21:30:11',1,'0000-00-00 00:00:00',NULL,NULL),(13,'踢掉在线用户','/shotOffOnlineUser.do ',6,6,1,0,'2018-06-15 21:35:16',1,'0000-00-00 00:00:00',NULL,NULL),(14,'日志管理','/sysLog.do',7,1,1,1,'2018-06-15 21:23:24',1,'0000-00-00 00:00:00',NULL,NULL),(15,'系统数据','/sysConfig.do',7,2,1,1,'2018-06-15 21:23:56',1,'0000-00-00 00:00:00',NULL,NULL);
...
) ENGINE=InnoDB AUTO_INCREMENT=6 DEFAULT CHARSET=utf8 COLLATE=utf8_bin COMMENT='系统全局配置信息表';

/*Data for the table `sys_config` */

insert  into `sys_config`(`id`,`config_item`,`config_name`,`config_value`,`value_type`,`config_note`,`create_userid`,`create_time`,`update_userid`,`update_time`) values ('1','image_file_max_size','图片大小限制（M）','10','integer','图片大小属性的最大值，单位 M','1','2018-11-20 21:23:57',NULL,'0000-00-00 00:00:00'),('2','video_file_max_size','视频大小限制（M）','200','integer','视频大小属性的最大值，单位 M','1','2018-11-20 21:23:57',NULL,'0000-00-00 00:00:00'),('3','image_file_suffix','图片文件的后缀名','jpg,bmp,jpeg,png,gif,tiff,eps','string','图片文件的后缀名，请使用英文逗号分割','1','2018-06-29 12:26:26','1','2018-07-04 22:23:15'),('4','video_file_suffix','视频文件的后缀名','mp4,flv,mpeg,mpg,ram,mov,avi,ra,rm,rmvb','string','视频文件的后缀名，请使用英文逗号分割','1','2018-07-04 21:45:29',NULL,'0000-00-00 00:00:00'),(5,'tonghu_api_url_prefix','潼湖项目API应用接口URL前缀','http://123.57.132.183:8080/tonghuapi','string','潼湖项目API应用接口URL前缀',1,'2018-06-29 12:26:26',NULL,'0000-00-00 00:00:00');
